...
public class DispatcherHandlerFactoryImpl implements DispatcherHandlerFactory {
    @Override
    public DispatcherHandler dispatcherHandler(ServerDeployContext<? extends ServerOptions> ctx) {
        return new DispatcherHandlerImpl(ctx.routeRegistry().orElse(null));
    }
}
