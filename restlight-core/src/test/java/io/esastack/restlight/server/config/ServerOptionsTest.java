...
class ServerOptionsTest {
    ...
    void testConfigure() {
        ...
        options.setBizThreads(BizThreadsOptionsConfigure.newOpts()
                .core(10)
                .max(20)
                .blockingQueueLength(30)
                .keepAliveTimeSeconds(40L)
                .configured());
        assertEquals(10, options.getBizThreads().getCore());
        assertEquals(20, options.getBizThreads().getMax());
        assertEquals(30, options.getBizThreads().getBlockingQueueLength());
        assertEquals(40L, options.getBizThreads().getKeepAliveTimeSeconds());
    }
    ...
    void testDefaultOpts() {
        ...
        assertEquals(def.getBizThreads().getCore(), options.getBizThreads().getCore());
        assertEquals(def.getBizThreads().getMax(), options.getBizThreads().getMax());
        assertEquals(def.getBizThreads().getBlockingQueueLength(),
                options.getBizThreads().getBlockingQueueLength());
        assertEquals(def.getBizThreads().getKeepAliveTimeSeconds(),
                options.getBizThreads().getKeepAliveTimeSeconds());
    }
