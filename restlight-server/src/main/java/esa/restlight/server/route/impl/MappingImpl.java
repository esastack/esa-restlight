    ...
    public MappingImpl consumes(String... consumes) {
        if (consumes != null && consumes.length > 0) {
            ...
            for (String c : consumes) {
                tmp.add(MediaType.valueOf(c).value());
            }
            return new MappingImpl(this.name,
                    this.path,
                    this.method,
    ...
    public MappingImpl produces(String... produces) {
        if (produces != null && produces.length > 0) {
            ...
            for (String p : produces) {
                tmp.add(MediaType.valueOf(p).value());
            }
            return new MappingImpl(this.name,
                    this.path,
                    this.method,
    ...
    public String toString() {
        if (str == null) {
            ...
            if (path != null && path.length > 0) {
                fragments.add("path=" + Arrays.asList(path).toString());
            }
            if (method != null && method.length > 0) {
                fragments.add("method=" + Arrays.asList(method).toString());
            }

            if (params != null && params.length > 0) {
                fragments.add("params=" + Arrays.asList(params).toString());
            }

            if (headers != null && headers.length > 0) {
                fragments.add("headers=" + Arrays.asList(headers).toString());
            }

            if (consumes != null && consumes.length > 0) {
                fragments.add("consumes=" + Arrays.asList(consumes).toString());
            }

            if (produces != null && produces.length > 0) {
                fragments.add("produces=" + Arrays.asList(produces).toString());
            }
